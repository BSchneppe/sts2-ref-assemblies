...
if test (count $argv) -lt 1
    echo "Usage: $argv[0] <release|beta> [beta-suffix]"
    exit 1
end
set -l release_type $argv[1]
...
set -l beta_suffix ""
if test "$release_type" = "beta"
    if test (count $argv) -ge 2
        set beta_suffix $argv[2]
    else if test -f "$release_info"
        set beta_suffix (grep '"commit":' "$release_info" | sed -E 's/.*"([a-f0-9]+)".*/\1/')
        if test -z "$beta_suffix"
            echo "Beta builds require a unique suffix (commit hash not found)."
            exit 1
        end
    else
        echo "Beta builds require a unique suffix (commit hash not found)."
...
if test -f "$release_info"
    ...
    if test "$release_type" = "beta"
        set game_version "$game_version-beta-$beta_suffix"
    end
    echo "Detected Game Version: $game_version"
    # Update .csproj version
    set -l csproj "BSchneppe.Sts2.ReferenceAssemblies.csproj"
    if test -f "$csproj"
