...
if [ -f "$RELEASE_INFO" ]; then
    ...
    sed -i '' -E "s|<Version>[^<]+</Version>|<Version>$GAME_VERSION</Version>|" BSchneppe.Sts2.ReferenceAssemblies/BSchneppe.Sts2.ReferenceAssemblies.csproj
    else
    ...
    OUTPUT_DIR="BSchneppe.Sts2.ReferenceAssemblies/ref/net9.0"
    mkdir -p "$OUTPUT_DIR"
